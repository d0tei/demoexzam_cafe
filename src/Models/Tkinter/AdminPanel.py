...
def admin():
    def status_false(id):
        button_login.configure(text = "Уволен", background='gray')
    ...
    panel_admin.title("Панель админстратора")
    title_window = Label(panel_admin, text='Панель админстратора', font=("Times New Roman", 24))
    ...
    for row in list_user:
        if row.status:
            ...
            button_login = Button(panel_admin,
                                 ...
                                 background = 'red',
                                 foreground = 'white',
                                 #command = lambda: user.update_status(row.id)
                                 command = lambda id = row.id: status_false(row.id))

            status_tutle = Label(panel_admin, text=row.id)
            status_tutle.grid(column=4,row=count_row,padx=0, pady=1)
